...
struct graph
{
    ...
    // BFS
    void bfs(vector<long> &result, long start, long weight = 1)
    {
        vector<long> dist(vertexCount, -1); // map distance menyimpan distance antara starting node ke node yang dituju
        vector<bool> visited(vertexCount, false);
        queue<long> q;
        ...
        while (!q.empty())
        {
            ...
            for (auto vertex : adjList[temp])
            {
                ...
                {
                    visited[v] = true;
                    dist[v] = dist[temp] + weight;  // jarak dari start
                    q.push(v);
                }
            }
        }
...
int main()
{
    ...
    while (t--)
    {
        ...
        graph g;
        g.init(n + 1); // +1 biar indexing dari 1
